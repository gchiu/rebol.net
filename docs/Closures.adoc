...
A *function* takes a set of arguments, computes with them, then returns
a result. There are several types of functions, depending on how they
are implemented.
...
the results may be problematic.
...
`  >> add2: closure [c d] `link:c_+_d[`c` `+` `d`] +
`  >> do add2 1 2` +
`  == 3`

This works because the variables of a closure remain valid, even outside
the closure after it has been called. Such variables have _indefinite
extent_. They are not limited to the lifetime of the function.

Note, however, that this luxury provided by closures is not without its
...
that context. You can read more about the benefits and costs of closures
in REBOL 3 link:R2_vs._R3_Contexts[here].
...
http://en.wikipedia.org/wiki/Closure_(computer_science)[More about
function closures]
